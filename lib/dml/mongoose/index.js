...
MongooseDB.prototype.connect = function(db) {
  ...
  connectionString += buildQueryParams();
...
function buildQueryParams() {
  ...
  if(typeof authSource === 'string' && authSource !== '' ){
    queryParams.push({key: 'authSource', val: authSource});
  }

  var useSSL = configuration.getConfig('dbUseSSL');
  if(typeof useSSL === 'string' && useSSL === 'Y' ){
    queryParams.push({key: 'ssl', val: 'true'});
  }
